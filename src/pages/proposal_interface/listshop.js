...
export function ListShop(props){
    ...
    function selectedShopCallback(e){
        if(e.actionType=='shopClick'){
            ...
            if(Object.keys(cart).length==0 || (key && cart[key].shop==e.selectedShop.id) )
                history.push('/frontend/myshop/proposalinterface/listproductproposal/?shop='+e.selectedShop.id,e.selectedShop)
            else{
                setOpen(true)
    ...
    function continueWithPreviousShop(e){
        ...
        for(key in cart)
            break
        history.push('/frontend/myshop/proposalinterface/listproductproposal/?shop='+cart[key].shop)
        setSelectedShop(null)
    }
    function ContinewWithNewShop(e){
        setOpen(false)
        cartDispatch({type:'Default'})
        history.push('/frontend/myshop/proposalinterface/listproductproposal/?shop='+selectedShop.id,selectedShop)
        setSelectedShop(null)
    } 
    return(
        <>
